...
| Phase                    | 모드          | 이유                                                                |
| ------------------------ | ------------- | ------------------------------------------------------------------- |
| Phase 2 (아트디렉션)     | 서브 에이전트 | art-director 단일이 스타일 + 프롬프트 생성               |
...
1. `_workspace/01_story.md` 존재 여부 확인. 없으면 사용자에게 요청
2. `book/index.html` 존재 여부 확인
3. 실행 모드 결정:
   - `book/index.html` 존재 + 사용자가 부분 수정 요청 → **부분 재실행** (해당 에이전트만 호출)
   - `book/index.html` 존재 → **새 실행**, 기존 `_workspace/` 와 `book/` 를 각각 `_workspace_{timestamp}/`, `book_{timestamp}/` 로 보관 후 `_workspace/`에 `01_story.md`를 이동
...
1. `Agent(name: art-director, subagent_type: art-director, model: opus, prompt: "{theme}에 대해 동화 시나리오 11장면 작성 후, 일관된 비주얼 스타일 + 12개 영문 프롬프트 생성")`
