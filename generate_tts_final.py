#!/usr/bin/env python3
"""Generate TTS using google.generativeai (deprecated but working)."""

import json
import sys
import time
import subprocess
from pathlib import Path
from dotenv import load_dotenv
import os

load_dotenv()

def generate_tts_for_book(book_dir, output_dir="audio"):
    """Generate TTS for all scenes."""
    try:
        import google.generativeai as genai
    except ImportError:
        print("Installing google-generativeai...")
        subprocess.check_call([sys.executable, "-m", "pip", "install", "-q", "google-generativeai"])
        import google.generativeai as genai

    api_key = os.getenv('GEMINI_API_KEY', '').strip("'\"")
    if not api_key:
        print("❌ GEMINI_API_KEY not found")
        return False

    genai.configure(api_key=api_key)

    book_json_path = Path(book_dir) / "book.json"
    if not book_json_path.exists():
        print(f"❌ Book file not found: {book_json_path}")
        return False

    with open(book_json_path, 'r', encoding='utf-8') as f:
        book_data = json.load(f)

    audio_path = Path(book_dir) / output_dir
    audio_path.mkdir(exist_ok=True)
    print(f"📁 Audio: {audio_path}\n")

    pages = book_data.get('pages', [])
    scenes = [p for p in pages if p.get('type') == 'scene']
    total = len(scenes)

    emotion_to_tone = {
        '조심스러운 호소, 은밀함': 'Say cautiously',
        '위협, 압도': 'Say menacingly',
        '공포, 굴복': 'Say fearfully',
    }

    success = 0
    for idx, page in enumerate(scenes, 1):
        page_num = page.get('number', idx)
        title = page.get('title', '')
        body = page.get('body', '')

        tone = page.get('voice_tone', '')
        if not tone and page.get('emotion'):
            tone = emotion_to_tone.get(page.get('emotion'), '')

        text = f"{title}. {body}".strip()
        if tone:
            text = f"{tone}: {text}"

        if not text:
            continue

        audio_file = audio_path / f"page_{page_num:02d}.mp3"
        print(f"[{idx}/{total}] {title[:35]}...", end=" ", flush=True)

        try:
            model = genai.GenerativeModel("gemini-2.5-flash-preview-tts")
            response = model.generate_content(text)

            audio_bytes = None
            if hasattr(response, 'audio_data'):
                audio_bytes = response.audio_data
            elif hasattr(response, 'parts'):
                for part in response.parts:
                    if hasattr(part, 'audio_data'):
                        audio_bytes = part.audio_data
                        break

            if audio_bytes and len(audio_bytes) > 100:
                with open(audio_file, 'wb') as f:
                    f.write(audio_bytes)
                size = audio_file.stat().st_size / 1024
                print(f"✓ ({size:.1f}KB)")
                success += 1
            else:
                print("✗ (empty)")

        except Exception as e:
            print(f"✗ ({str(e)[:50]})")

    print(f"\n✅ Done! {success}/{total} files generated")
    return success > 0


if __name__ == "__main__":
    book = "01-rabbit-tale"
    if len(sys.argv) > 1:
        book = sys.argv[1]

    book_dir = Path(__file__).parent / "books" / book
    if not book_dir.exists():
        print(f"❌ Not found: {book_dir}")
        sys.exit(1)

    success = generate_tts_for_book(book_dir)
    sys.exit(0 if success else 1)
