#!/usr/bin/env python3
"""Generate TTS using Gemini API - Fixed interactions.create() format."""
...
def generate_tts_for_book(book_dir, output_dir="audio"):
    """Generate TTS for all scenes using Gemini API interactions API."""
    try:
    ...
    print(f"🎤 Generating TTS for {total} scenes using Gemini TTS API...\n")
    ...
    for idx, page in enumerate(scenes, 1):
        ...
        try:
            # Use google.genai SDK with text_to_speech method
            response = client.models.generate_content(
                model="gemini-3.1-flash-tts-preview",
                contents=[{
                    "role": "user",
                    "parts": [{"text": narration_text}]
                }]
            )
    ...
    if rate_limit_hit:
        print(f"\n⚠️  Rate limited. Quota may be depleted. Wait and retry later.")
    else:
